...
task Filter {
  ...
  Float hc_contamination = if run_contamination && hasContamination == "YES" then (if contamination_major == 0.0 then select_first([contamination_minor]) else 1.0 - select_first([contamination_major])) else 0.0
  Float max_contamination = if defined(verifyBamID) && select_first([verifyBamID]) > hc_contamination then select_first([verifyBamID]) else hc_contamination
